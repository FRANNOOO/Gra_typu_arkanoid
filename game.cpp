    ...
    pilka(420.f, 400.f, 200.f, 150.f, 8.f) // x,y,vx,vy,radius
{
    ...
    ROZMIAR_BLOKU_Y = s.blockH;
    ROZMIAR_BLOKU_X = LevelBuilder::blockWidth(width, s);

    bloki = LevelBuilder::buildDefault(width, s);
}
...
int Game::update(sf::Time dt) {
    ...
    //sprawdzenie przegranej lub wygranej
    if (pilka.getY() - pilka.getRadius() > height) {return 1;}

    if (wszystkieBlokiZniszczone()) {return 2;}
